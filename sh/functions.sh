function activeContext() {
    task rc.hooks:off context rc.defaultwidth:200 | grep -oP "(?<=read  ).*(?=yes)"
}

function tasktrack() {
    task add "$@"
    #ID=$(task +LATEST | grep -oP "^\d+")
    ID=$(task +LATEST rc.hooks:off rc.report.next.columns=id rc.report.next.labels=ID rc.verbose=nothing next)
    echo "Last ID $ID"
    task "${ID}" timew start pomodoro
}

function timewlogged() {
    timew s $(task ${1:-$(task rc.hooks:off nextid rc.verbose=nothing)} id rc.verbose=nothing) :all
}
